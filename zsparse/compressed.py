...
class cs_matrix():
    ...
    def __init__(self,arg,shape=None,dtype=None, compressor=None,
        ...
        if not all(isinstance(i, zarr.Array) for i in [data, indices, indices]):
            self.data = zarr.array(data,chunks=data_chunks,dtype=dtype,store=data_store,compressor=compressor)
            self.indices = zarr.array(indices,chunks=data_chunks,store=indices_store,compressor=compressor)
            self.indptr = zarr.array(indptr,chunks=indptr_chunks,store=indptr_store,compressor=compressor)
        else:
            self.data = data
